...
def product_quantity_ordered():
    prod = Product.objects.all().annotate(total=Sum("orderproduct__quantity")).filter(total__gte=1).order_by("-total")
    res = []
    for ordered_p in prod:
        res.append(f"Quantity ordered of {ordered_p.name}: {ordered_p.total}")
    return "\n".join(res)
...
def ordered_products_per_customer():
    ...
    for order in orders:
        ...
        for ordered in order.orderproduct_set.all():
            res.append(f"- Product: {ordered.product.name}, Category: {ordered.product.category.name}")
...
# def filter_products():
#     query = Q(is_avalaible=True) & Q(prices__gte=3.00)
#     filtered = Product.objects.filter(query).order_by('-price', 'name')
#     return filtered
...
def give_discount():
    ...
    return "\n".join(f'{product.name}: {product.price}lv.' for product in Product.objects.filter(is_available=True).order_by('-price', 'name'))
