...
def find_authors_nationalities():
    ...
    return '\n'.join([f"{str(auth)} is {auth.nationality}" for auth in authors])
...
def filter_authors_by_nationalities(author_nationality:str):
    auths = Author.objects.filter(nationality=author_nationality).order_by('first_name', 'last_name')
    return '\n'.join([a.biography if a.biography else f"{a.first_name} {a.last_name}" for a in auths])


# print(filter_authors_by_nationalities('American'))

# def filter_authors_by_birth_year(year1, year2):
#     authors = Author.objects.filter(birth_date__range=[str(year1) + "-01-01", str(year2) + "-01-01"]).order_by('-birth_date')
#     return '\n'.join(f'{a.birth_date}: {a.first_name} {a.last_name}' for a in authors)


def filter_authors_by_birth_year(first_year, second_year):
    filtered_and_ordered_authors = Author.objects.filter(birth_date__range=[str(first_year) + "-01-01", str(second_year+1) + "-01-01"]).order_by('-birth_date')

    return "\n".join(f"{a.birth_date}: {str(a)}" for a in filtered_and_ordered_authors)
...
#     rev = Review.objects.filter(reviewer_name=reviewer_name)
#     for r in rev:
#         r.reviewer_name = new_name
#     Review.objects.bulk_update(rev, ['reviewer_name'])
